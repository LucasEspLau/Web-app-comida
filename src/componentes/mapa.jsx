...
export function MapComponent() {
    useEffect(() => {
        ...
        script.src = `https://maps.googleapis.com/maps/api/js?key=${process.env.VITE_GOOGLE_API_KEY}&callback=initMap&libraries=geometry`;
        script.async = true;
        script.defer = true;
      ...
      // Funciones para manejar la lógica del mapa
    function initMap() {
    ...
    // Eventos de botón como funciones de React
    const reloadMap = () => {
        initMap();
    };

    const confirmLocation = () => {
        const confirmedLocation = markerUsuario.getPosition();
        searchCooks(confirmedLocation);
    };

    const searchCooksButton = () => {
        // Lógica para buscar cocineros
    };
    ...
    const handleSubmit = (e) => {
        ...
        navigate('/cocineros'); // Cambia '/ruta-despues-login' por la ruta a la que deseas redirigir
    }

  return (
    <div className='mapaArea'>
        <div id="map"></div>
        <div className="button-container">
            <button id="reloadButton" onClick={reloadMap}>Recargar Mapa</button>
            <button id="confirmLocationButton" onClick={confirmLocation}>Buscar Cocineros</button>
            <button id="searchCooksButton" onClick={handleSubmit}>Ver Cocineros</button>
        </div>
    </div>
  );
}
